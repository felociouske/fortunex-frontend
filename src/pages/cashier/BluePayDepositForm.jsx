...
import { bluepayAPI } from "../../api/bluepay";
...
/**
 * BluePay M-Pesa STK Push deposit flow.
 *
 * This is a SEPARATE payment rail from the Daraja paybill flow in
 * MpesaDepositForm.jsx -- kept in its own file/component on purpose so
 * you can drop this one and go back to Daraja-only later without
 * touching that file at all.
 *
 * Lifecycle: idle -> "sending" (awaiting BluePay to accept the push)
 * -> "waiting" (prompt is on the user's phone) -> "success" | "failed".
 *
 * Same WebSocket-first, one-shot-fallback pattern as the Daraja flow:
 * the backend broadcasts on the same `deposit_user_<id>` channel and
 * reuses the `checkout_request_id` event key (BluePay's checkout id IS
 * Safaricom's native ws_CO_... id, same shape Daraja gives directly),
 * so useDepositSocket needs no changes to support both rails.
 */
const FALLBACK_TIMEOUT_MS = 20000;
const GIVE_UP_TIMEOUT_MS = 60000;

export default function BluePayDepositForm() {
  const [amount, setAmount] = useState("");
  const [phone, setPhone] = useState("");
  const [rate, setRate] = useState(null);
  const [stage, setStage] = useState("idle"); // idle | sending | waiting | success | failed
  const [trackingId, setTrackingId] = useState(null); // BluePay checkout_request_id
  ...
  // never drifts from what the backend will actually charge. This
  // endpoint is shared infrastructure (not Daraja-specific), safe to reuse.
  useEffect(() => {
  ...
  // React to the WebSocket event, once we have a tracking id to match against.
  useEffect(() => {
    if (!lastEvent || !trackingId) return;
    if (lastEvent.checkout_request_id !== trackingId) return; // some other deposit's event

    if (lastEvent.status === "APPROVED") {
      resolveSuccess(`Deposit of $${amount} confirmed via BluePay.`);
    } else if (lastEvent.status === "FAILED") {
      resolveFailure(lastEvent.result_desc);
    }
    // if still PROCESSING, ignore -- keep waiting
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lastEvent, trackingId]);
  ...
  const handleSubmit = async (e) => {
    ...
    setStage("sending");
    try {
      const { data } = await bluepayAPI.stkPush({ amount, phone_number: phone });
      setTrackingId(data.checkout_request_id);
      setStage("waiting");

      // Fallback: if the WebSocket hasn't resolved things within 20s,
      // ask the backend to actively re-check with BluePay once.
      fallbackTimerRef.current = setTimeout(async () => {
        try {
          const res = await bluepayAPI.depositStatus(data.checkout_request_id);
      ...
      setStage("idle");
      setError(err.response?.data?.detail || "Unable to start the BluePay payment.");
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-md">
      ...
      {kesPreview && (
        <p className="text-sm text-fx-text-dim -mt-3">
          You will pay approximately <span className="text-fx-text font-medium">KES {kesPreview}</span> via M-Pesa (BluePay).
        </p>
      )}
